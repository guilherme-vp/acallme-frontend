...
export const useSchedule = (date: Date = new Date()) => {
	const { user, role } = useStoreon('user', 'role')
	const intl = useIntl()
	...
	const { data } = useQuery('schedules', () =>
		fetchSchedules({
			specialistId: role === RolesEnum.Specialist && user ? user.id : undefined,
			patientId: role === RolesEnum.Specialist && user ? user.id : undefined,
		...
		.map((_, index) => {
			...
			const mockedWeekDays: HoursRange[] = Array(daysInWeek)
				...
				.map((__, day): HoursRange => {
					const aimedDay = set(addDays(firstDayOfWeek, day), { hours: hour, minutes: 0 })

					const formattedHour = format(aimedDay, 'HH:mm')

					const scheduleExists = data?.find(
						({ rangeStart }) => rangeStart.getTime() === aimedDay.getTime()
					)
